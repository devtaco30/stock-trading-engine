...
- **어떻게(대안·결정·트레이드오프)**:
  ...
  **결정(Jack)**: Aeron Cluster 채택 안 함, fork2 손조립(A).
  - 후자(Cluster) 이익이 적은 이유 세 가지(⚠️ "HA는 불필요"라는 축약은 부정확 — 아래로 대체):
    - ① **death는 이미 덮인다.** ADR-031:36이 페일오버 = 안정 신원 재시작 + replay(구현)로 정했다. 프로세스가 죽으면 같은 신원으로 재시작 → PV+저널 replay → 재개. 대가는 재시작 동안 몇 초 그 계좌 불가용이고 ADR-031:48이 그 대가를 명시적으로 수용한다. 즉 Cluster 없이도 death 복구는 된다. ADR-031이 "불필요"로 둔 건 HA 자체가 아니라 장중 무중단·즉시 인수(warm standby)·상시 코디네이터·오토스케일이고(상황 가정 :24-25), warm standby는 "선택적 뒤 최적화"로 미뤄 뒀다(:36 survivor-takeover).
    - ② **warm standby(fork2)는 운영 필요가 아니라 학습 때문에 만든다.** 규모 프레임 금지 — 판단 기준은 학습 가치다. 그런데 Cluster를 쓰면 그 학습(손으로 짠 standby 저널 tailing·승격·fencing)이 블랙박스로 숨는다. v2 핵심 학습(락 없는 엔진·저널·스냅샷)은 이미 벌었고 수치까지 났는데, Cluster는 그 메커니즘까지 가린다. HA만 얹는 것도 아니라 엔진을 ClusteredService로 다시 앉히는 척추 재작성이다(병용 불가, either/or). 기존 v2 서사("락 vs single-writer, 손으로 만든 깊이")도 무뎌진다.
    - ③ **Cluster의 다중 노드 복제 내구성은 한 머신에선 성립 안 한다.** 복제 내구성의 본체는 노드 셋에 나눠 적어 디스크 하나가 날아가도 사는 것인데, 지금은 한 머신에서 프로세스를 띄우므로 3노드를 띄워도 디스크가 하나다. 치르는 값(모든 입력에 과반 확인 왕복 — 측정된 v2 p50 1.9ms는 그게 없는 수치)은 그대로다.
