...
/**
 * 계좌 엔진에 들어온 이벤트 하나의 불변 스냅샷(2b-1) — matching {@code JournaledOrder}의 계좌판.
 * 매칭과 달리 주문(BUY·SELL)만이 아니라 체결·정산까지 전 타입({@link EventType}의 다섯 값 전부)을
 * 담는다 — 계좌 상태를 만드는 모든 입력을 순서대로 남겨야 복구(2b-2 리플레이)가 성립하기 때문이다.
 *
 * <p>{@link AccountEvent}는 링버퍼가 재사용하는 가변 슬롯이라, 다음 발행 때 필드가 덮어써진다.
 * 그 참조를 그대로 저널에 담으면 나중에 값이 바뀌어 기록이 훼손된다 — 그래서 발행 시점의 값을
 * 이 불변 record로 복사해 담는다({@link AccountJournalEventHandler} 참고).</p>
 *
