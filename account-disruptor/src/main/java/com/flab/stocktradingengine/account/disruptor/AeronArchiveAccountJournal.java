...
/**
 * 계좌 저널을 Aeron Archive 로 durable 하게 내보내는 {@link AccountJournal} 구현(2b-1b).
 *
 * <p>{@link #append}는 {@link AccountJournalEventHandler}(저널 게이팅 단계)의 단일 스레드에서만
 ...
 * <p>저널은 리플레이(2b-2)가 계좌 상태를 되살리는 유일한 근거라 하나라도 빠지면 복구가 어긋난다.
 * 그래서 {@link AeronMatchingOrderSender}(②-b, best-effort 1회)와 달리 여기는 {@code offer} 가
 * 성공(반환값 ≥ 0)할 때까지 {@link BackoffIdleStrategy}로 재시도한다. NOT_CONNECTED·BACK_PRESSURED·
 * ADMIN_ACTION 은 일시적이라 재시도로 넘어가지만, CLOSED·MAX_POSITION_EXCEEDED 는 이 스트림이
 * 복구 불가 상태라는 뜻이라 예외를 던진다 — {@link AccountExceptionHandler}가 그 예외를 받아
 * 저널 소비자 스레드를 fail-fast 로 멈춘다(오염된 상태 위에서 계속 진행하지 않는다).</p>
 *
 * <p>호출 스레드가 하나뿐이라({@code append}가 저널러 단일 스레드에서만 불린다) 인코딩 버퍼를
 * 필드로 재사용해도 안전하다.</p>
 */
public class AeronArchiveAccountJournal implements AccountJournal {
    ...
    public AeronArchiveAccountJournal(ExclusivePublication journalPublication) {
        ...
        while ((result = journalPublication.offer(buffer, 0, length)) < 0) {
            if (result == Publication.CLOSED || result == Publication.MAX_POSITION_EXCEEDED) {
                throw new IllegalStateException(
                    "계좌 저널 스트림이 복구 불가 상태입니다(offer 결과=" + result + "): 리플레이 신뢰가 깨지므로 소비자를 멈춥니다");
            }
            idleStrategy.idle();
        }
    }

    /** 읽기(read-back)는 2b-2 리플레이가 Archive 에서 직접 하므로 여기서는 지원하지 않는다. */
    @Override
    public List<AccountJournalEntry> entries() {
        throw new UnsupportedOperationException("Aeron Archive 저널의 read-back 은 2b-2 리플레이에서 다룬다");
    }

    /** 발행 스트림의 현재 위치. 스냅샷(2d-2)이 "여기까지는 스냅샷에 담겼다"로 함께 저장한다. */
    @Override
