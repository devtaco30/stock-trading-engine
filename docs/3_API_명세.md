...
2. 각 계좌별로 결제 처리:
   ...
   - **변제 대용 처리 (회전매매 지원)** (미구현: 매도 대금을 담보로 인정하는 로직은 코드에 없습니다):
...
3. 결제 성공 (가용 자금 >= 필요한 결제 잔금):
   ...
   - 계좌 상태가 IN_ARREARS인 경우, 미수금이 모두 해결되었는지 확인 후 ACTIVE로 변경 — 미구현(상태 전환을 부르는 코드 없음)
4. 결제 실패 (가용 자금 < 필요한 결제 잔금):
   - 부족 금액을 미수금으로 생성
   - 계좌 상태를 IN_ARREARS로 변경 — 미구현(상태 전환을 부르는 코드 없음)
   - 미수금 발생일 기록
   - 홀딩된 증거금은 그대로 유지 (미수금 상환 시 사용)

### 강제 청산 배치

> **미구현.** 아래는 설계만 적어 둔 것이고 이 배치를 실행하는 코드는 없습니다.
