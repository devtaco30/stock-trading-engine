...
- 테스트에 함정이 하나 있었다. 매칭과 계좌가 둘 다 멱등이라, 복구할 때 position을 무시하고 0부터 읽어도 최종 상태는 같은 값으로 수렴한다. 그래서 "복구 뒤 상태가 맞다"는 단언은 `readFrom`이 position을 제대로 쓰는지 검증하지 못한다. 리플레이어를 직접 호출해 돌아온 엔트리 목록으로 "position 이전 것이 실제로 빠졌는지"를 본다.
