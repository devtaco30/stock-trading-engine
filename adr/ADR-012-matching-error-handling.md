...
`matching-disruptor` 는 링버퍼(RingBuffer) 소비자 체인 `handleEventsWith(journal).then(matcher)` 로 주문을 journal에 기록한 뒤 매칭한다. 이 소비자들은 별도 스레드에서 무한 루프로 이벤트를 처리하는데, 처리 중 예외가 나면 두 종류로 성격이 다르다.
...
| 복구 | 별도 개입 없이 계속 동작(문제 원인 파악이 늦어짐) | 재시작 + journal replay 필요(이후 단위) |
...
- 예상 못한 오류로 소비자가 멈춘 뒤의 복구는 프로세스 재시작 + journal replay로 처리한다. replay 자체(재유도·체크포인트)는 이번 단위 범위 밖이며 이후 단위에서 다룬다.

## 결과

- `matching-disruptor/src/main/.../MatchingExceptionHandler.java` 추가, `MatchingEngine` 생성자에서 명시적으로 등록.
- `JournalGatingTest` 로 journal 핸들러(`JournalEventHandler`)가 매처(`MatchingEventHandler`)보다 항상 먼저 도는 것(체결 시점엔 이미 journal에 기록돼 있는 것)을 검증했다. 이 게이팅이 fail-fast 정책의 전제다 — 매칭이 멈춰도 그 직전까지 들어온 주문은 journal에 남아 있어야 재시작 후 replay로 복구할 수 있다.
- **인메모리 journal의 한계**: `InMemoryJournal` 은 JVM 프로세스 메모리에만 있다. fail-fast 로 소비자를 멈춘 원인이 JVM 크래시라면 journal 자체도 함께 사라지므로, 지금 구조로는 "재시작 + journal replay"가 실제로 복구 수단이 되지 못한다. 복구가 실제로 되게 하려면 파일 기반(또는 그 이상의 영속) journal과 주기적 스냅샷이 필요하며, 이는 `Journal` 인터페이스로 이미 분리해 둔 자리에 이후 단위에서 구현한다.
- 향후 확장으로, 결정적 비즈니스 오류로 폐기된 이벤트를 그냥 버리지 않고 별도 기록(dead-letter journal)에 남겨 사후 분석·재현에 쓰는 방안을 고려할 수 있다. 이번 단위에서는 로그로만 남긴다.
