...
- 상태: 채택
- 관련: ADR-018(호스트 프레임워크) · ADR-019(매칭 내구성) · ADR-033(하이브리드 복원) · Kafka 완전 제거 시도(ADR-032 네타, `decision_records/single-stack-aeron-archive.md`, ADR-033이 되돌림)
...
- 2026-09-14 Kafka 완전 제거 결정(ADR-032 네타, `decision_records/single-stack-aeron-archive.md`)이 "정산까지 Aeron으로 통일"을 시도했다가 정산 경로에서 Kafka 프로듀서의 비동기·durable 전달을 손으로 재구현하게 되는 문제를 만나 되돌렸고, ADR-033이 이 ADR의 선(핫패스 Aeron / off-path Kafka)을 재확인했다.
- 실측: 3-JVM(api·account-worker·matching-worker)을 UDP로 띄운 라이브 왕복은 로그로 확인(`4905c7b`). 부하 수치는 [C7 리포트 삽입 예정 — 2026-09-16 측정].
