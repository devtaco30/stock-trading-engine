...
- **Aeron vs Kafka**: Aeron 은 브로커·디스크가 없어 지연이 마이크로초(µs)급이지만 기본적으로 저장하지 않는다(휘발). Kafka 는 디스크+복제로 내구성을 주는 대신 ms 급. 속도 차이의 본체는 인코딩(JSON vs 바이너리)이 아니라 브로커·디스크·복제의 유무다. 그래서 대안 1(Kafka+바이너리)로는 지연이 크게 줄지 않는다.
...
대안 2 를 택한다. Aeron 을 주문 전달 계층으로 도입하되, 코덱은 **수동 바이너리로 시작**(SBE 는 유예 — 흐름 학습이 우선)하고 통신은 **IPC 로 시작**(UDP 는 이후 단위). 내구성은 새로 만들지 않고 기존 저널러(Unit 2, 저널 리플레이 복구)가 책임진다. Kafka 는 제거가 아니라 **정산(돈) 경로로 유지**한다.
