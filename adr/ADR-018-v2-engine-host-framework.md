...
- **복구 시간의 병목은 상태 재구성이다.** 새 인스턴스가 샤드를 넘겨받으면 인메모리 상태(매칭=호가창, order-engine=계좌 예약·잔고)를 스냅샷 + journal replay로 재구성해야 한다. 프레임워크 부팅은 JVM 수 초 vs 네이티브 ~50ms 차이지만, 상태 재구성은 계좌·주문 볼륨에 따라 수십 초가 될 수 있어 부팅 최적화는 복구 시간의 일부만 줄인다.
